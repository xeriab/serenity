...
public:
    void init()
    {
        ...
        auto begin = data1.begin();
        auto end = data1.end();
        
        std::generate(begin, end, []() { return Util::rand() & 1; });
    }
...
private:
    ...
    [[nodiscard]] i8 const& get1(int x, int y) const { return data1[index(x, y)]; }
    [[maybe_unused]] [[nodiscard]] i8 const& get2(int x, int y) const { return data2[index(x, y)]; }
...
private:
    ...
    void create4x4()
    {
        ...
        if (Util::rand() % min<int>(1, 100 / prob) == 0) {
...
public:
    [[maybe_unused]] void set_transform(double scale, double theta)
    {
    ...
    [[nodiscard]] int get_pixel(int x, int y, double power) const
    {
